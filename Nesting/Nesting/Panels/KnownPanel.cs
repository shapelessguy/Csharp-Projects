...
{
    public class KnownPanel : System.Windows.Forms.Panel
    {
        ...
        {
            foreach (Reference x in parent.unk_panel.getReferences())
            {
                if (x.text.Name == (string)e.Data.GetData(DataFormats.Text))
                {
                    ...
                    {
                        if (x.text.Name == (string)e.Data.GetData(DataFormats.Text))
                            parent.MoveToSeason(x, (SeasonPanel)sender);
                        return;
                    }
                }
            }
            foreach (SeasonPanel pan in parent.known_panel.getSeasonList())
            {
                foreach (Reference x in pan.getReferences())
                    if (x.text.Name == (string)e.Data.GetData(DataFormats.Text))
                    {
                        if (!((SeasonPanel)sender).getReferences().Contains(x))
                        {
        ...
        private void remove_panel_Click(object sender, EventArgs e)
        {
            ...
            {
                parent.MoveToUnknownPanel(seasonList[seasonList.Count - 1].getReferences()[i]);
            }
